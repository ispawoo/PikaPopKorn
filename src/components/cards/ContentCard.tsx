...
export function ContentCard({ content, progress, subtitle }: ContentCardProps) {
  return (
    <Link href={`/content/${content.slug}`}>
      <motion.div 
        whileHover={{ scale: 1.05 }}
        whileTap={{ scale: 0.95 }}
        className="relative group w-32 md:w-40 lg:w-48 aspect-[2/3] rounded-2xl overflow-hidden bg-zinc-900 border border-white/5 shrink-0"
      >
        {content.poster_url ? (
          <Image
        ...
        <div className="absolute bottom-0 left-0 right-0 p-3 flex flex-col justify-end">
          <h3 className="text-sm md:text-base font-bold text-white line-clamp-1 leading-tight mb-0.5">
            {content.title}
          </h3>
          
          {subtitle ? (
            <p className="text-[10px] md:text-xs text-zinc-400 line-clamp-1 mb-2">{subtitle}</p>
          ) : (
            <div className="flex items-center gap-2 text-[10px] md:text-xs text-zinc-400 opacity-0 group-hover:opacity-100 transition-opacity duration-300 mb-1">
